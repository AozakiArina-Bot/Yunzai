...
export default class Yunzai extends EventEmitter {
  ...
  serverAuth (req) {
    ...
    for (const i of req.app?.skip_auth || []) {
      if (req.originalUrl.startsWith(i)) { return req.next() }
    }
  ...
  serverHandle (req) {
    ...
    for (const i of ["query", "body"]) {
      if (Object.keys(req[i]).length) { msg[i] = req[i] }
    }
  ...
  async serverExit (req) {
    ...
    if (process.env.app_type === "pm2") { await util.exec("pnpm stop") }
    process.exit(1)
  }
  ...
  async run () {
    ...
    util.makeLog("info", `连接地址：${logger.blue(`${this.url.replace(/^http/, "ws")}/`)}${logger.cyan(`[${Object.keys(this.wsf)}]`)}`, "WS")
  ...
  prepareEvent (data) {
    ...
    if (!data.reply) {
      if (data.group?.sendMsg) { data.reply = data.group.sendMsg.bind(data.group) } else if (data.friend?.sendMsg) { data.reply = data.friend.sendMsg.bind(data.friend) }
    }
  }
