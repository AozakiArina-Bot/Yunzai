...
export default new class {
  makeLogID (id) {
    if (!id) return cfg.bot.log_align || "Yunzai"
    if (!cfg.bot.log_align) return id
    if (typeof id !== "string") id = this.String(id)
    const length = (cfg.bot.log_align.length - id.length) / 2
    if (length > 0) { id = `${" ".repeat(Math.floor(length))}${id}${" ".repeat(Math.ceil(length))}` } else if (length < 0) { id = id.slice(0, cfg.bot.log_align.length - 1) + "." }
    return id
  }
  ...
  getCircularReplacer () {
    const _this_ = this; const ancestors = []
  ...
  Loging (data, opts = cfg.bot.log_object) {
    if (typeof data === "string") {} else if (!opts) { data = this.StringOrNull(data) } else {
  ...
  async exec (cmd, opts = {}) {
    return new Promise(resolve => {
      ...
      if (Array.isArray(cmd)) { execFile(cmd.shift(), cmd, opts, callback) } else { exec(cmd, opts, callback) }
    })
  }
