...
import plugin, { SymbolContextHandler, SymbolContextPlugin } from "./plugin.js"
...
class PluginsLoader {
  ...
  async importPlugin (file, packageErr) {
    ...
    try {
      ...
      for (const i of await Promise.allSettled(pluginArray)) {
        if (i?.status && i.status !== "fulfilled") { Bot.makeLog("error", [`插件加载错误 ${logger.red(file.name)}`, i], "Plugin") }
      }
    } catch (error) {
  ...
  async deal (e) {
    ...
    for (const i of priority) {
      ...
      const contexts = [
        { data: i.plugin.getContext(), isGroup: false },
        { data: i.plugin.getContext(false, true), isGroup: true }
      ]
      if (contexts.some(({ data }) => !lodash.isEmpty(data))) {
        let ret
        let handled = false
        const plugin = Object.assign(new i.class(e), { e })
        for (const { data, isGroup } of contexts) {
          if (lodash.isEmpty(data)) continue
          for (const fnc in data) {
            const contextPlugin = data[fnc]?.[SymbolContextPlugin]
            const contextHandler = data[fnc]?.[SymbolContextHandler]
            const handler = contextHandler || (typeof plugin[fnc] === "function" ? plugin[fnc] : null)
            if (typeof handler !== "function") {
              Bot.makeLog("error", `${e.logText}${logger.blue(`[${plugin.name}(${fnc})]`)}${logger.red("[上下文方法不存在，等待超时清理]")}`, false)
              continue
            }
            handled = true
            ret ||= await handler.call(Object.assign(contextHandler ? (contextPlugin || plugin) : plugin, { e }), data[fnc])
          }
        }
        if (!handled) continue
  ...
  filtEvent (e, v) {
    ...
    for (const i in event) {
      if (event[i] === "*") { newEvent.push(event[i]) } else { newEvent.push(e[eventMap[i]]) }
    }
      ...
      for (const i of e.message) {
        switch (i.type) {
          case "text":
            ...
            break
          case "image":
            if (Array.isArray(e.img)) { e.img.push(i.url) } else { e.img = [i.url] }
            break
          case "at":
            if (i.qq == e.self_id) { e.atBot = true } else { e.at = i.qq }
            break
          case "reply":
            e.reply_id = i.id
            if (e.group?.getMsg) { e.getReply = () => e.group.getMsg(e.reply_id) } else if (e.friend?.getMsg) { e.getReply = () => e.friend.getMsg(e.reply_id) }
            break
          case "file":
            e.file = i
            break
  ...
  reply (e) {
    ...
    e.reply = async (msg = "", quote = false, data = {}) => {
      ...
      if (quote && e.message_id) {
        if (Array.isArray(msg)) { msg.unshift(segment.reply(e.message_id)) } else { msg = [segment.reply(e.message_id), msg] }
      }
  ...
  async count (e, type, msg) {
    if (cfg.bot.msg_type_count) {
      for (const i of Array.isArray(msg) ? msg : [msg]) { await this.saveCount(e, `${type}:${i?.type || "text"}`) }
    }
