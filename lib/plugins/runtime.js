...
for (let plugin of ["genshin", "Mys-plugin"]) {
  try {
    gsCfg = (await import(`../../plugins/${plugin}/model/gsCfg.js`)).default
    MysApi = (await import(`../../plugins/${plugin}/model/mys/mysApi.js`)).default
    MysInfo = (await import(`../../plugins/${plugin}/model/mys/mysInfo.js`)).default
    NoteUser = (await import(`../../plugins/${plugin}/model/mys/NoteUser.js`)).default
    MysUser = (await import(`../../plugins/${plugin}/model/mys/MysUser.js`)).default
    break
  } catch {}
}
try {
...
export default class Runtime {
  ...
  async getUid () {
    let e = this.e
    let game = e?.game || (e?.isSr ? "sr" : "gs")
    let uidReg = /^(18|[1-9])[0-9]{8}$/
    let getUserUid = user => {
      let uid = user?.getUid?.(game)
      if (uid) return String(uid)
      let uidList = user?.getUidList?.(game) || []
      if (uidList.length === 1 && uidList[0]?.uid) {
        return String(uidList[0].uid)
      }
      return false
    }

    if (e?.uid && uidReg.test(String(e.uid))) {
      return String(e.uid)
    }

    if (e?.at && NoteUser?.create) {
      let atUser = await NoteUser.create(e.at)
      return getUserUid(atUser)
    }

    if (!this.user && NoteUser?.create) {
      await this.initUser()
    }
    return getUserUid(this.user) || this.uid || false
  }
  ...
  async render (plugin, path, data = {}, cfg = {}) {
    ...
    data = {
      ...
      /** miao 相关参数 **/
      copyright: `Created By Yunzai<span class="version">${Version?.yunzai}</span> `,
